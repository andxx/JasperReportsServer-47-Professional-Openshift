...
var ControlsBase = {
    ...
    buildParamsFromJQueryParams: function (jQueryParamsObject) {
        var formattedObject = ControlsBase.convertJQueryParams(jQueryParamsObject);
        return ControlsBase.buildParams(formattedObject);
    },

    // Convert object produced by jquery <string, <real type> || array<real type>>
    // to the map of string arrays <string, array<string>>
    convertJQueryParams: function (jQueryParamsObject) {
        var json = {};
        if (_.isObject(jQueryParamsObject) && !_.isEmpty(jQueryParamsObject)) {
            _.each(jQueryParamsObject, function(paramValue, paramName) {
                if (_.isArray(paramValue)) {
                    json[paramName] = _.map(paramValue, function(value) {
                        return String(value);
                    });
                } else if (_.isString(paramValue) || _.isNumber(paramValue) || _.isDate(paramValue) || _.isBoolean(paramValue) || _.isNull(paramValue)) {
                    json[paramName] = [String(paramValue)];
                } else {
                    json[paramName] = [''];
                }
            });
