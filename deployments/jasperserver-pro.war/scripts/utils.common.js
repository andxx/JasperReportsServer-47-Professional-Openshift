...
var ValidationModule = {
    ...
    showError: function(element, errorMessage) {
        var errorMsg = $(element.parentNode).select(layoutModule.MESSAGE_WARNING_PATTERN)[0];
        if (!errorMsg){
        	errorMsg = document.createElement('span');
        	var classes = layoutModule.MESSAGE_WARNING_PATTERN.split('.');
        	for (var i = 0; i<classes.length; i++){
        		errorMsg.addClassName(classes[i].trim());
        	}
        	element.parentNode.appendChild(errorMsg);
        }
        errorMsg.update(errorMessage);
